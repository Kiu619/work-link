...
import type { Recruiter, RecruiterListParams } from '@/types/recruiter'
...
const mockGetRecruiterListApi = (
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      // Filter data
      const filteredData = MOCK_RECRUITERS.filter((recruiter) => {
        const matchSearch =
          !search ||
          recruiter.fullName.toLowerCase().includes(search) ||
          recruiter.idCardNumber.toLowerCase().includes(search) ||
          recruiter.placeOfResidence.toLowerCase().includes(search) ||
          recruiter.placeOfOrigin.toLowerCase().includes(search)

        const matchGender = !gender || recruiter.gender === gender
...
const RecruiterList = () => {
  ...
  const {
    ...
    queryKey: ['recruiters', pagination.current, pagination.pageSize, debouncedSearch, genderFilter],
    queryFn: async () => {
      // ============================================
      ...
      // MOCK DATA - Xóa khi có API thật
      // ============================================
      const response = await mockGetRecruiterListApi({
        page: pagination.current,
        maxSize: pagination.pageSize,
        search: debouncedSearch,
        gender: genderFilter,
  ...
  const stats = {
    total: totalElement,
    male: recruiters.filter((r) => r.gender === 'Nam').length,
    female: recruiters.filter((r) => r.gender === 'Nữ').length,
  }
  ...
  const handleViewDetail = (record: Recruiter) => {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <PageHeader
        ...
        subtitle="Quản lý thông tin nhà tuyển dụng đã đăng ký trên hệ thống"
        actions={
          <>
      ...
      {/* Statistics Cards */}
      <RecruiterStats total={stats.total} male={stats.male} female={stats.female} />
